...
function showCategory(category) {
    products[category].forEach(product => {
        productsBlock.innerHTML = productsBlock.innerHTML + getProductMarkup(product);
    });
}
...
function getProductMarkup(product) {
    let markup = `<div class="product"><div>${product.name}</div><img src="${product.imageUrl}" alt=""><div>${product.price}</div><a href="https://example.com/producs/${product.id}">Подробнее</a></div>`;
    return markup;
}